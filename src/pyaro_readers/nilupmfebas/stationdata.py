...
from .helpers import calc_climatology, isnumeric, isrange, to_datetime64
from .metastandards import STANDARD_META_KEYS, StationMetaData
from .time_resampler import TimeResampler
from .tstype import TsType
from .units_helpers import convert_unit, get_unit_conversion_fac
...
class StationData(StationMetaData):
    ...
    def get_unit(self, var_name):
        ...
        if not var_name in self.var_info:
            raise MetaDataError(
                f"Could not access variable metadata dict for {var_name}."
            )
        try:
    ...
    def convert_unit(self, var_name, to_unit):
        ...
        try:
            ...
            tst = None
        data = convert_unit(
            data, from_unit=unit, to_unit=to_unit, var_name=var_name, ts_type=tst
        )
        ...
        for key in self.STANDARD_COORD_KEYS:
            ...
            if val is not None:
                if not isnumeric(val):
                    raise MetaDataError(
                        f"Station coordinate {key} must be numeric. Got: {val}"
                    )
                output[key] = val
            else:
                val = self[key]
    ...
    def _merge_meta_item(self, key, val):
        ...
        try:
            if isinstance(current_val, dict):
                if not same_type:
                    raise ValueError(
                        f"Cannot merge meta item {key} due to type mismatch"
                    )
    ...
    def plot_timeseries(
        self, var_name, add_overlaps=False, legend=True, tit=None, **kwargs
    ):
        """
        ...
        **kwargs
        ...
        if tit is None:
            try:
                tit = self.get_meta(force_single_value=True, quality_check=False)[
                    "station_name"
                ]
            except Exception:
