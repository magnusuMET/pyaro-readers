...
def get_topo_data(
    ...
    if topodata_loc is None:
        if topo_dataset in const.SUPPLDIRS and os.path.exists(
            const.SUPPLDIRS[topo_dataset]
        ):
...
def get_topo_altitude(
    lat, lon, topo_dataset="srtm", topodata_loc=None, try_etopo1=True
):
    """Retrieve topographic altitude for a certain location
    ...
    return get_topo_data(
        lat,
        lon,
        topo_dataset=topo_dataset,
        topodata_loc=topodata_loc,
        try_etopo1=try_etopo1,
    )(lat, lon)


def calc_distance(
    lat0, lon0, lat1, lon1, alt0=None, alt1=None, auto_altitude_srtm=False
):
