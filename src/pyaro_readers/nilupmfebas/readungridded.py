...
class ReadUngridded:
    ...
    def data_dirs(self, val):
        ...
        if len(dsr) < 2 and isinstance(val, str):
            ...
            raise ValueError(
                f"Invalid input for data_dirs ({val}); needs to be a dictionary."
            )
    ...
    def _find_read_class(self, data_id):
        ...
        for _cls in self.SUPPORTED_READERS:
            if data_id in _cls.SUPPORTED_DATASETS:
                return _cls
        raise NetworkNotImplemented(
            f"Could not find reading class for dataset {data_id}"
        )
