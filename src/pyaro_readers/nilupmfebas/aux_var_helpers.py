...
def _calc_od_helper(
    ...
    if not od_ref in data:
        if od_ref_alt is None and not od_ref_alt in data:
            raise AttributeError(
                f"No alternative OD found for computation of {var_name}"
            )
...
def vmrx_to_concx(
    data, p_pascal, T_kelvin, vmr_unit, mmol_var, mmol_air=None, to_unit=None
):
    """
...
def concx_to_vmrx(
    data, p_pascal, T_kelvin, conc_unit, mmol_var, mmol_air=None, to_unit=None
):
    """
