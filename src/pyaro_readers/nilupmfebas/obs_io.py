...
class AuxInfoUngridded:
    ...
    def check_status(self):
        ...
        if isinstance(self.aux_merge_how, str):
            ...
            self.aux_merge_how = dict(
                zip(self.vars_supported, [self.aux_merge_how] * nv)
            )
        ...
        for var in self.vars_supported:
            if not var in self.aux_requires:
                raise ValueError(
                    f"Variable {var} is not defined in attr aux_requires..."
                )
                ...
                for _var in var_info:
                    ...
                    if fc > self.MAX_VARS_PER_METHOD:
                        raise NotImplementedError(
                            "So far only 2 variables can be combined..."
                        )
