...
from .obs_io import OBS_WAVELENGTH_TOL_NM
...
from .varnameinfo import VarNameInfo
    ...
    **kwargs
    ...
    def _check_input_var_name(var_name):
        if "3d" in var_name:
            ...
            raise ValueError(
                f"invalid variable name {var_name}. Must not contain underscore"
            )
        return var_name
    ...
    def lower_limit(self):
        ...
        warnings.warn(
            "Old name for attribute minimum", DeprecationWarning, stacklevel=2
        )
        return self.minimum
    ...
    def upper_limit(self):
        ...
        warnings.warn(
            "Old name for attribute maximum", DeprecationWarning, stacklevel=2
        )
        return self.maximum
    ...
    def parse_from_ini(self, var_name=None, cfg=None):
        ...
        if cfg is None:
            ...
            raise ValueError(
                f"invalid input for cfg, need config parser got {type(cfg)}"
            )
