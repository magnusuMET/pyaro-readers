...
class ReadEbas(ReadUngriddedBase):
    ...
    def _get_var_cols(self, ebas_var_info, data):
        ...
        *ebas_config.ini* where section names are Aerocom namespace and
        ...
        if len(col_matches) == 0:
            raise NotInFileError(
                f"Variable {ebas_var_info.var_name} could not be found in file"
            )
            ...
            for prefcomp in ebas_var_info.component:
                ...
                col_matches = [
                    ...
                    if prefcomp == data.var_defs[colnum].name
                ]
                break
        return col_matches
    ...
    def _find_best_data_column(
        self, cols, ebas_var_info, file, check_units_on_multimatch=True
    ):
        ...
        try:
            ...
            if not code in self.TS_TYPE_CODES:
                raise NotImplementedError(
                    f"Cannot handle EBAS resolution code {tres_code}"
                )
    ...
    def compute_additional_vars(self, data, vars_to_compute):
        ...
        for var in vars_to_compute:
            ...
            data.var_info[var].update(
                self.get_ebas_var(var)
            )  # self._loaded_ebas_vars[var]
    ...
    def _read_files(self, files, vars_to_retrieve, files_contain, constraints):
        ...
        for i in tqdm(range(num_files), disable=None):
            ...
            if "station_name_orig" in station_data:
                metadata[meta_key]["station_name_orig"] = station_data[
                    "station_name_orig"
                ]
            ...
            for var_count, var in enumerate(append_vars):
                ...
                data_obj._data[start:stop, data_obj._LATINDEX] = station_data[
                    "latitude"
                ]
                data_obj._data[start:stop, data_obj._LONINDEX] = station_data[
                    "longitude"
                ]
                data_obj._data[start:stop, data_obj._ALTITUDEINDEX] = station_data[
                    "altitude"
                ]
