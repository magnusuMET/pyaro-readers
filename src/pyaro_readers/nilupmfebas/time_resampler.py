...
from .helpers import isnumeric, resample_time_dataarray, resample_timeseries
...
class TimeResampler:
    ...
    def _gen_idx(self, from_ts_type, to_ts_type, min_num_obs, how):
        ...
        if len(idx) == 0 or not idx[-1][0] == to_ts_type.val:
            try:
                last_entry = self._get_idx_entry(
                    last_from, to_ts_type, min_num_obs, how
                )
            except (TemporalResolutionError, ValueError):
        ...
        **kwargs
        ...
        if from_ts_type is None:  # native == unknown
            ...
            for to_ts_type, mno, rshow in _idx:
                ...
                data_out = self.fun(
                    data_out, freq=freq, how=rshow, min_num_obs=mno, **kwargs
                )
                aggrs.append(rshow)
