...
def seconds_in_periods(timestamps, ts_type):
    ...
    if ts_type >= TsType("monthly"):
        if ts_type == TsType("monthly"):
            days_in_months = np.array(
                [timestamp.days_in_month for timestamp in timestamps]
            )
        ...
        raise NotImplementedError(
            "Only yearly, monthly and daily frequencies implemented."
        )
...
def merge_station_data(
    ...
    stats, is_3d, has_errs = _check_stats_merge(
        stats, var_name, pref_attr, fill_missing_nan
    )
...
def calc_climatology(
    s, start, stop, min_count=None, set_year=None, resample_how="mean"
):
...
def resample_time_dataarray(arr, freq, how=None, min_num_obs=None):
    ...
    if not isinstance(arr, xr.DataArray):
        ...
        raise DataDimensionError(
            "Cannot resample time: input DataArray has no time dimension"
        )
    ...
    try:
        ...
        raise ResamplingError(
            f"Invalid aggregator {how} for temporal resampling of DataArray..."
        )
    arr = aggfun(dim="time")
...
def same_meta_dict(
    meta1, meta2, ignore_keys=["PI"], num_keys=NUM_KEYS_META, num_rtol=1e-2
):
...
def to_datetime64(value):
    ...
    if isinstance(value, np.datetime64):
        ...
        try:
            ...
            raise ValueError(
                f"Failed to convert {value} to datetime64 objectError: {repr(e)}"
            )
...
def get_constraint(
    lon_range=None, lat_range=None, time_range=None, meridian_centre=True
):
...
def get_time_rng_constraint(start, stop):
    ...
    t_lower = iris.time.PartialDateTime(
...
def get_max_period_range(periods):
    ...
    stop = max(
        int(per.split("-")[1]) if len(per.split("-")) > 1 else int(per)
        for per in periods
    )
