...
from .geodesy import get_country_info_coords
from .helpers import (
    isnumeric,
    merge_station_data,
    same_meta_dict,
...
from .helpers_landsea_masks import get_mask_value, load_region_mask_xr
from .mathutils import in_range
from .metastandards import STANDARD_META_KEYS
    ...
    **Example 2**: 3 variables, 5 files, 30 timestamps, but each variable
    ...
    def add_station_data(
        ...
        if data_idx is None:
            ...
            raise ValueError(
                f"Cannot add data at data index {data_idx}, index already exists"
            )
    ...
    def to_station_data(
        ...
        if isinstance(vars_to_convert, str):
            ...
            if len(vars_to_convert) == 0:
                raise DataCoverageError(
                    "UngriddedData object does not contain any variables"
                )
        ...
        if isinstance(meta_idx, str):
            ...
            meta_idx = self.find_station_meta_indices(
                meta_idx, allow_wildcards_station_name
            )
            ...
            try:
                ...
                if ts_type_preferred is not None:
                    if "ts_type" in stat["var_info"][vars_to_convert[0]].keys():
                        if TsType(
                            stat["var_info"][vars_to_convert[0]]["ts_type"]
                        ) < TsType(ts_type_preferred):
                            continue
        ...
        try:
            ...
            if not "variables" in meta or meta["variables"] in (None, []):
                raise VarNotAvailableError(
                    "Metablock does not contain variable information"
                )
    ...
    def get_variable_data(
        ...
        if isinstance(variables, str):
            variables = [variables]
        all_stations = self.to_station_data_all(
            variables, start, stop, freq=ts_type, **kwargs
        )
        result = {}
        num_stats = {}
    ...
    def set_flags_nan(self, inplace=False):
        ...
        if not self.has_flag_data:
            raise AttributeError(
                "Ungridded data object does not contain flagged data points"
            )
        if inplace:
            obj = self
        else:
    ...
    def _find_meta_matches(self, negate=None, *filters):
        ...
        *filters
        ...
        if negate is None:
            ...
            raise ValueError(
                f"Invalid input for negate {negate}, need list or str or None"
            )
        meta_matches = []
        totnum = 0
    ...
    def filter_region(
        self, region_id, check_mask=True, check_country_meta=False, **kwargs
    ):
        ...
        **kwargs
        ...
        return self.filter_by_meta(
            longitude=region.lon_range, latitude=region.lat_range
        )
    ...
    def filter_by_meta(self, negate=None, **filter_attributes):
        ...
        **filter_attributes
        ...
        if len(meta_matches) == len(self.metadata):
            logger.info(
                f"Input filters {filter_attributes} result in unchanged data object"
            )
            return self
    ...
    def code_lat_lon_in_float(self):
        ...
        self.coded_loc = self._data[:, self._LONINDEX] * 10 ** (
            3 * self._LOCATION_PRECISION
    ...
    def merge(self, other, new_obj=True):
        ...
        if not isinstance(other, UngriddedData):
            raise ValueError(
                f"Invalid input, need instance of UngriddedData, got: {type(other)}"
            )
        if new_obj:
            obj = self.copy()
        else:
                ...
                for var in check_vars_available:
                    try:
                        if not var in meta["variables"]:
                            logger.debug(
                                f"No {var} in data of station {name} ({meta['data_id']})"
                            )
                            ok = False
                ...
                for meta_idx_other, meta_other in other.metadata.items():
                    if meta_other["station_name"] == name:
                        if _check_vars:
                            for var in check_vars_available:
                                try:
                                    ...
                                    Exception
                                ):  # attribute does not exist or is not iterable
                                    ok = False
                        if ok and check_coordinates:
                            ...
                            dist = np.linalg.norm(
                                (dlat * lat_len, dlon * lat_len * lon_fac)
                            )
    ...
    def find_common_data_points(self, other, var_name, sampling_freq="daily"):
        ...
        for idx_this, idx_other in common.items():
            ...
            date_nums_this = (
                dtimes_this.astype("datetime64[s]").astype("M8[D]").astype(int)
            )
            date_nums_other = (
                dtimes_other.astype("datetime64[s]").astype("M8[D]").astype(int)
            )
        ...
        **kwargs
        ...
        if var_name is None:
            ...
            if not isinstance(var_name, str):
                ...
                raise ValueError(
                    f"Input variable {var_name} is not available in dataset "
                )
            info_str = var_name
    ...
    def __contains__(self, key):
        ...
        if not isinstance(key, str):
            raise ValueError(
                "Need string (e.g. variable name, station name, instrument name"
            )
    ...
    def __next__(self):
        ...
        try:
            ...
            logger.warning(
                f"No variable data in metadata block {self._idx}. "
                f"Returning empty StationData"
            )
            return StationData()
